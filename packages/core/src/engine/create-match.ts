...
function getPieceLockTicks(mode: MatchMode, phase: MatchPhase): number {
  if (mode !== 'solo') {
...
function drawSoloPiece(player: PlayerState): PieceKind {
  const weightedSoloBag: PieceKind[] = [
    'ridge',
    'ridge',
    'corner',
    'corner',
  ...
  const activeBag: PieceKind[] = [...weightedSoloBag];
  if (waterUnlocked) {
  ...
  for (let attempt = 0; attempt < 6; attempt += 1) {
    const bag = placedPieces < 4 ? openingBag : activeBag;
